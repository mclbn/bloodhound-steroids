...
def do_insert(driver, user, others):
    res_count = 0
    query = """
    MATCH (a:User), (b:User)
...
def do_samepass_user_file(driver, user_files):
    ...
    for user in users:
        updated += do_insert(driver, '%s@%s' % (user, options.domain),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(add_help = True, description = "Give performance enhancing drugs to bloodhound.")

    parser.add_argument('-l', '--list', action='store_true', default=False, help='list available modules')
    parser.add_argument('-m', '--module', action='store', help='module to use')
    parser.add_argument('--domain', action='store', help='the related domain')
    parser.add_argument('--user-file', action='append', help='a list of user names (you can specify multiple files)')
    parser.add_argument('--nt-file', action='store', help='a NT dump file (NTDS.DIT) to parse')
    parser.add_argument('--neo4j-host', '-n', action='store', help='target neo4j host')
    parser.add_argument('--port', '-p', action='store', default=7687, help='target neo4j port (default: 7474)')
    parser.add_argument('--user', '-u', action='store', default='neo4j', help='target neo4j username')
    parser.add_argument('--secret', '-s', action='store', help='target neo4j secret')
